...
cif_path = caget("abtest:ioc-hkl:cif_path")
...
#TODO write output to IOC directory, not whereever phoebus shell is 
##### parse .cif and set lattice parameters to IOC #####
lattice = parse_cif_lattice_params(cif_path)

caput("abtest:ioc-hkl:latt_a", lattice['a'])
caput("abtest:ioc-hkl:latt_b", lattice['b'])
caput("abtest:ioc-hkl:latt_c", lattice['c'])
caput("abtest:ioc-hkl:latt_alpha", lattice['alpha'])
caput("abtest:ioc-hkl:latt_beta", lattice['beta'])
caput("abtest:ioc-hkl:latt_gamma", lattice['gamma'])

##### scattering intensities calculation #####
cif2hkl_bin = '/usr/bin/cif2hkl'
wavelength = caget("abtest:ioc-hkl:wlen_RBV")
...
caput("abtest:ioc-hkl:intensities", output)
